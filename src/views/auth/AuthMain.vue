...
<ion-page>
    <ion-content :fullscreen="true">
        ...
        <p style="text-align: left; padding: 0px 22px 0px; margin-top: 1rem; margin-bottom: 0;  align-items: flex-start; min-width: 100%; font-size: 34px; font-weight: 600;">Here we go!</p>
        <p style="text-align: left; padding: 0px 22px 0px; padding-top: 2%; margin: 0; transform-origin: left center; align-items: flex-end; min-width: 100%; font-size: 18px; font-weight: 200;">Log in so we can provide you with a unique meditation experience.</p>
            ...
            const userResponse = await GoogleAuth.signIn()
                    ...
                    axios.get(globaldata.api.hostname + "service/auth_withGoogle", { params:
                            ...
                            if(response.data.status == "okay"){
                                ...
                                this.message_modal_text = `There was an error logging in with your Google Account. Try again or use a different login method.\n\nInformation: ` + JSON.stringify(response)
                                this.message_modal_isOpen = true;
                            }
                        }).catch(function(error){
                            parent_this.message_modal_text = `The server is temporarily unavailable.\n\nInformation: ` + error
